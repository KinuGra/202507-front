...
const CreateProblemPage = () => {
  ...
  // genresの変更をLocalStorageに保存する
  useEffect(() => {
    if (!isInitialLoad) {
      try {
        localStorage.setItem('problem-genres', JSON.stringify(genres));
      } catch (error) {
        console.error("Failed to save genres to localStorage", error);
      }
    }
  }, [genres, isInitialLoad]);

  // Genre Dialog States
  const [isGenreDialogOpen, setIsGenreDialogOpen] = useState(false);
  const [newGenreName, setNewGenreName] = useState('');
  
  // Edit Genre Dialog States
  ...
  // Create Problem Dialog States
  const [isProblemDialogOpen, setIsProblemDialogOpen] = useState(false);
  const [newQuestion, setNewQuestion] = useState('');
  const [newAnswer, setNewAnswer] = useState('');

  // Edit Problem Dialog States
  ...
  const handleCreateProblem = () => {
    ...
    const newProblem: Problem = { id: Date.now(), question: newQuestion, answer: newAnswer };
    const updatedGenres = genres.map(g => 
      g.id === selectedGenre.id 
        ? { ...g, problems: [...g.problems, newProblem] } 
        : g
    );
    setGenres(updatedGenres);
    const updatedSelectedGenre = updatedGenres.find(g => g.id === selectedGenre.id);
    setSelectedGenre(updatedSelectedGenre || null);
    setNewQuestion('');
    setNewAnswer('');
  ...
  const handleUpdateProblem = () => {
    ...
    const updatedProblems = selectedGenre.problems.map(p => 
      p.id === editingProblem.id 
        ? { ...p, question: problemQuestionToEdit, answer: problemAnswerToEdit } 
        : p
    );
    const updatedGenres = genres.map(g => 
      g.id === selectedGenre.id 
        ? { ...g, problems: updatedProblems } 
        : g
    );
    setGenres(updatedGenres);
    const updatedSelectedGenre = updatedGenres.find(g => g.id === selectedGenre.id);
    ...
    return (
      <div className="h-full w-full max-w-4xl p-4 md:p-6">
        <div className="grid grid-cols-1 gap-4 mb-24">
          {genres.map((genre) => (
            <Card key={genre.id} className="w-full">
        ...
        <Dialog open={isGenreDialogOpen} onOpenChange={setIsGenreDialogOpen}>
          ...
          <DialogContent><DialogHeader><DialogTitle>新しいジャンルを作成</DialogTitle></DialogHeader><Input value={newGenreName} onChange={(e) => setNewGenreName(e.target.value)} placeholder="ジャンル名" /><DialogFooter><Button onClick={handleCreateGenre}>完了</Button></DialogFooter></DialogContent>
        </Dialog>
        <Dialog open={isEditGenreDialogOpen} onOpenChange={setIsEditGenreDialogOpen}>
          <DialogContent><DialogHeader><DialogTitle>ジャンル名を編集</DialogTitle></DialogHeader><Input value={genreNameToEdit} onChange={(e) => setGenreNameToEdit(e.target.value)} placeholder="新しいジャンル名" /><DialogFooter><Button onClick={handleUpdateGenreName}>保存</Button></DialogFooter></DialogContent>
        </Dialog>
      </div>
    );
  }
  ...
  return (
    <div className="h-full w-full max-w-4xl p-4 md:p-6">
      ...
      <div className="grid grid-cols-1 gap-4 mb-24">
        {selectedGenre.problems.length === 0 ? (
          ...
          selectedGenre.problems.map((problem) => (
            <Card key={problem.id} className="w-full cursor-pointer hover:bg-gray-50" onClick={() => handleOpenEditProblemDialog(problem)}>
      ...
      <Dialog open={isProblemDialogOpen} onOpenChange={setIsProblemDialogOpen}>
        <DialogTrigger asChild><Button className="fixed bottom-20 right-8 rounded-full w-16 h-16 shadow-lg"><Plus className="h-8 w-8" /></Button></DialogTrigger>
        <DialogContent><DialogHeader><DialogTitle>新しい問題を作成</DialogTitle></DialogHeader><div className="grid gap-4 py-4"><Label htmlFor="question">問題文</Label><Textarea id="question" value={newQuestion} onChange={(e) => setNewQuestion(e.target.value)} placeholder="問題文を入力" /><Label htmlFor="answer">回答</Label><Input id="answer" value={newAnswer} onChange={(e) => setNewAnswer(e.target.value)} placeholder="回答を入力" /></div><DialogFooter><Button onClick={handleCreateProblem}>保存</Button></DialogFooter></DialogContent>
      </Dialog>
      <Dialog open={isEditProblemDialogOpen} onOpenChange={setIsEditProblemDialogOpen}>
        <DialogContent><DialogHeader><DialogTitle>問題を編集</DialogTitle></DialogHeader><div className="grid gap-4 py-4"><Label htmlFor="edit-question">問題文</Label><Textarea id="edit-question" value={problemQuestionToEdit} onChange={(e) => setProblemQuestionToEdit(e.target.value)} placeholder="問題文を入力" /><Label htmlFor="edit-answer">回答</Label><Input id="edit-answer" value={problemAnswerToEdit} onChange={(e) => setProblemAnswerToEdit(e.target.value)} placeholder="回答を入力" /></div><DialogFooter><Button onClick={handleUpdateProblem}>保存</Button></DialogFooter></DialogContent>
      </Dialog>
    </div>
  );
};
