// ログインによって発行される認証セッションの処理を行うルート

// 処理内容
// 1. クエリ code の存在チェック
// 2. クッキー pkce_v（code verifier）取得
// 3. /oauth2/token へ POST し、認可コードを ID トークンに交換
// 4. ID トークンを検証（署名・audience）
// 5. 認証セッションクッキーを発行し、pkce_v を削除 → /profile にリダイレクト
